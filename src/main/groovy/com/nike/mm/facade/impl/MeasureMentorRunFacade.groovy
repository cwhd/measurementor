...
class MeasureMentorRunFacade implements IMeasureMentorRunFacade {
	...
	@Autowired Set<IMeasureMentorBusiness> measureMentorBusinesses
	
	@Autowired IMeasureMentorJobsConfigBusiness measureMentorConfigBusiness
	...
	void runJobId(String jobid) {
		...
		if (this.measureMentorRunBusiness.isJobRunning(jobid)) {
			this.jobHistoryBusiness.save([jobid:jobid, startDate: startDate, endDate: new Date(), success:'false', status: "jobrunning", comments: ('Job already running for the jobid:' + jobid)] as JobHistory);
		...
		try {
			...
			if (!mmcvDto.errors.isEmpty()) {
				this.jobHistoryBusiness.save([jobid:jobid, startDate: startDate, endDate: new Date(), success:'false', status: "error", comments: mmcvDto.getMessageAsString()] as JobHistory);
			} else {
				...
				//TODO: Error handling.
				this.jobHistoryBusiness.save([jobid:jobid, startDate: startDate, endDate: new Date(), success:false, status: "success", comments: ("Success for jobid: " + jobid )] as JobHistory);
			}
		} finally {
	...
	private MeasureMentorConfigValidationDto validateTheConfigFilesAndPlugins(def configs) {
		...
		configs.each { config ->
			...
			if (mmb == null) {
				...
				mmcvDto.runnableMmbs.add([measureMentorBusiness: mmb, config: config] as RunnableMeasureMentorBusinessAndConfigDto)
			}
		}
		return mmcvDto;
	}
