...
class JiraDataService {
    ...
    def getProjects() {
        ...
        for(def i : json) {
            logger.info(i.key)
            logger.info(i.name)
            logger.info(i.projectCategory?.name)
            logger.info(i.projectCategory?.description)
        }

        logger.info("GOT ALL THE PROJECTS!")
    }
                    ...
                    for (def t : h.items) {
                        ...
                        if(t.field == "status") { //NOTE get the progression for churn
                            if(taskStatusMap[t.fromString] > taskStatusMap[t.toString]){
                                ...
                                moveForward++
                                movedToDevList.add(UtilitiesService.cleanJiraDate(h.created)) //when it was moved to dev
                            }
                        } else if(t.field == "assignee"){ //NOTE get everyone that worked on this issue, or at least was assigned to it
                            if(t.toString) {
